...
`THINGSDB_WS_PORT` | *disabled* | WebSocket port  _(`9270` is commonly used when enabled)_.
`THINGSDB_WS_KEY_FILE` | *none* | Path to private key file _(required for Secure WebSockets)_.
`THINGSDB_WS_CERT_FILE` | *none* | Path to certificate file _(required for Secure WebSockets)_.
