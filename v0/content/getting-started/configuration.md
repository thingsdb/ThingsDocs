...
`THINGSDB_RESULT_SIZE_LIMIT` | `20971520` | The result size limit is checked when packing properties for a thing. At this point, and only then, this data limit is checked and if exceeded, a [RESULT_TOO_LARGE](../../errors/#internal-errors) error will be returned. This limit is set in bytes and is used to prevent a huge amount of data, typically when a high `deep` value is used. The default value is `20MB`.
